...
describe("ERC1155StakeMiningV1.sol", function () {
  ...
  describe("distribute()", async () => {
    ...
    describe("ERC1155 Stake Mining Pool", async () => {
      ...
      it("should return the same result when it uses safeTransfer instead of stake", async () => {
        await testingERC1155
          .connect(alice)
          .safeTransferFrom(
            alice.address,
            erc1155StakeMining.address,
            0,
            5,
            []
          );
        ...
        await testingERC1155
          .connect(bob)
          .safeTransferFrom(bob.address, erc1155StakeMining.address, 0, 5, []);
        await goTo(10000); // 50 : 50 : 0 => 150 : 50 : 0
        await testingERC1155
          .connect(carl)
          .safeTransferFrom(carl.address, erc1155StakeMining.address, 0, 5, []);
        await goTo(10000); // 33 : 33 : 33 => 183 : 83 : 33
    ...
    it("should return the same result when it uses safeBatchTransfer instead of stake", async () => {
      await testingERC1155
        .connect(alice)
